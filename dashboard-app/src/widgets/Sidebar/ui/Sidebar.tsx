...
import { AppRole, useRole } from "@/features/access/RoleProvider";
...
export function Sidebar() {
  ...
  const links: Array<{ href:string; label:string; icon:typeof LayoutDashboard; roles:AppRole[] }> = [
    ...
    { href:"/validaciones", label:"Validar", icon:BadgeCheck, roles:["ADMIN","VALIDATOR"] },
  ...
  return <aside className="fixed inset-y-0 left-0 z-20 w-28 bg-[#1a73e8] rounded-r-3xl px-3 py-5 flex flex-col items-center shadow-lg">
    ...
    <button title="Cerrar sesión" className="mt-auto h-11 w-11 rounded-xl grid place-items-center text-blue-100 hover:bg-white/20 hover:text-white"><LogOut size={19}/></button>
  </aside>;
}
